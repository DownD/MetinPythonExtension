import net_packet,ui,chr,player,chat,background,app
    ...
    def MoveToMapPosition(self,finalPosition,mapName=None,listLinks=[],callback=None,maxDist=250):
        if len(listLinks) == 0:
            if mapName == None or mapName == background.GetCurrentMapName():
                OpenLog.DebugPrint("Moving to ("+str(finalPosition[0])+","+str(finalPosition[1])+") on " + str(background.GetCurrentMapName()))
            ...
            if len(listLinks) == 0:
                OpenLog.DebugPrint("Error no map found by the name: "+str(mapName))
                return None
