...
class InventoryDialog(ui.ScriptWindow):
	...
	def Sort(self):
		...
		log.DebugPrint("[SORT] - Number of actions to be processed: "+str(len(self.toSortMoveActions)))
...
def AddSellItems(slots,callback=None):
	"""
	Add items and start selling them.

	Args:
		slots ([list]): A list of slot indexes.
		callback ([function], optional): A function callback that will be called when it has finish the selling operation. Defaults to None.
	"""
	for slot in slots:	
...
def AddBuyItems(slots,callback=None):
	"""
	Add items and start buying them.
