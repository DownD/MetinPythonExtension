import ui,app,chat,chr,net,player,item,skill,time,game,shop,chrmgr,m2k_lib,net_packet
import background,constInfo,miniMap,wndMgr,math,uiCommon,grp,FileManager,UIComponents,Movement
...
class SettingsDialog(ui.ScriptWindow):
	...
	#	self.Board.AddFlag("float") 
		...
		#self.AttackSpeedButton = self.comp.Button(self.attackTab, '', 'Attack-Speed', 25, 60, self.SetAttackSpeed, 'm2kmod/Images/Shortcuts/attack_0.tga', 'm2kmod/Images/Shortcuts/attack_1.tga', 'm2kmod/Images/Shortcuts/attack_0.tga')
		#self.MoveSpeedButton = self.comp.Button(self.generalTab, '', 'Move-Speed', 25, 110, self.SetMoveSpeed, 'm2kmod/Images/Shortcuts/move_0.tga', 'm2kmod/Images/Shortcuts/move_1.tga', 'm2kmod/Images/Shortcuts/move_0.tga')
		...
		self.DmgMenuButton = self.comp.Button(self.attackTab, '', 'Damage Hacks', 133, 150, self.OpenDmgMenu,  'm2kmod/Images/General/dmg_0.tga', 'm2kmod/Images/General/dmg_1.tga', 'm2kmod/Images/General/dmg_0.tga')
  		self.OneHandedButton = self.comp.Button(self.attackTab, '', 'One-Handed', 190, 150, self.SetOneHand, 'm2kmod/Images/General/onehand_0.tga', 'm2kmod/Images/General/onehand_1.tga', 'm2kmod/Images/General/onehand_0.tga')
		self.TwoHandedButton = self.comp.Button(self.attackTab, '', 'Two-Handed', 245, 150, self.SetTwoHand, 'm2kmod/Images/General/twohand_0.tga', 'm2kmod/Images/General/twohand_1.tga', 'm2kmod/Images/General/twohand_0.tga')
		...
		self.WallHackBtn = self.comp.OnOffButton(self.generalTab, '', 'WallHack', 200, 130, image='m2kmod/Images/General/wall.tga',funcState=self.WallHackSwich,defaultValue=int(self.wallHack))
		...
		##PICKUP
		self.pickupButton,self.SlidePickupSpeed,self.speedPickupLabel = UIComponents.GetSliderButtonLabel(self.pickupTab,self.pickupSpeedSlide, '', 'Enable Pickup', 30, 18,image="m2kmod/Images/General/pickup.tga",funcState=self.OnPickupOnOff,defaultValue=int(self.pickUp),defaultSlider=float(self.pickUpSpeed/3.0))
	...
	def LoadSettings(self):
		#m2k_lib.DebugPrint("Loading Settings")
	...
	def SaveSettings(self):
		#m2k_lib.DebugPrint("Saving Settings")
...
#UI STUFF
	def UpdatePickFilterList(self):	
		...
		for filterItem in sorted(self.pickFilter):
			...
			if searchValue in name:
				self.PickfileListBox.AppendItem(m2k_lib.Item(str(filterItem)+" "+name))

	def UpdateSellFilterList(self):	
		for filterItem in sorted(self.sellItems):
			item.SelectItem(filterItem)
			name = item.GetItemName()
			self.ShopFileListBox.AppendItem(m2k_lib.Item(str(filterItem)+" "+name))
	...
	def WallHackSwich(self,val):
		if bool(val):
			self.wallHack = True
			net_packet.DisableCollisions()
		else:
			self.wallHack = False
			net_packet.EnableCollisions()

	#PICKUP
	def OnChangePickMode(self,val):
		self.excludeInFilter = val
		if not val:
			net_packet.ItemGrndOnFilter()
		else:
			net_packet.ItemGrndNotOnFilter()

	def delPickFilterItem(self,id):
		net_packet.ItemGrndDelFilter(id)
		self.pickFilter.remove(int(id))

	def addPickFilterItem(self,id):
		net_packet.ItemGrndAddFilter(id)
		self.pickFilter.add(int(id))
		
	def PickUp(self):
		if self.pickUp:
			val, self.pickUpTimer = m2k_lib.timeSleep(self.pickUpTimer,self.pickUpSpeed)
			if not val:
				return
			...
			allowedRange = max(self.pickUpRange,m2k_lib.MAX_PICKUP_DIST) 
			if dst <= allowedRange:
				#Teleport to item
				if dst >= m2k_lib.MAX_PICKUP_DIST:
...
class ItemListDialog(ui.Window):
	...
	def UpdateFileList(self,mode):
		...
		try:
			...
			m2k_lib.DebugPrint("Load Itemlist Error, you have so set the IDs manually")
			self.Close()
		for line in lines:
			...
			if mode == 1:
				if Index and str(Itemname) != "":
					self.PickfileListBox.AppendItem(m2k_lib.Item(Index +"  " + Itemname))
			elif mode == 2:
