import ui,chat,net,player,item,shop,FileManager
import uiCommon,m2k_lib,math
...
class InventoryDialog(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.DropSelectedButton = self.comp.Button(self.Board, 'Drop Selected', 'Drops all same Items like the selected one', 15, 333, self.DropItemSelected, 'd:/ymir work/ui/public/Large_button_01.sub', 'd:/ymir work/ui/public/Large_button_02.sub','d:/ymir work/ui/public/Large_button_03.sub')
	...
	def SellItemSelected(self):
		...
		if ItemIndex:
			...
			chat.AppendChat(7, "[m2k-Mod] No selected Items!")
			return
		SelectedItem = ItemIndex.GetText().split("    ")
	...
	def SellAll(self):
		for i in range(0,m2k_lib.MAX_INVENTORY_SIZE):
	...
	def Sort(self):
		...
		m2k_lib.DebugPrint("[SORT] - Number of actions to be processed: "+str(len(self.toSortMoveActions)))


	def OnUpdate(self):
		val, self.lastTime = m2k_lib.timeSleep(self.lastTime,self.TIME_WAIT)
		if val:
			...
			if len(self.toStackMoveActions) > 0:
				...
				m2k_lib.DebugPrint("[STACK] - Changing item position from "+ str(args[0]) + " to " + str(args[1]) + " count " + str(args[2]))
				net.SendItemMovePacket(args[0],args[1],args[2])
				return

			if len(self.toSortMoveActions) > 0:
				args = self.toSortMoveActions.pop(0)
				m2k_lib.DebugPrint("[SORT] - Changing item position from "+ str(args[0]) + " to " + str(args[1]) + " count " + str(args[2]))
				net.SendItemMovePacket(args[0],args[1],args[2])
				return
...
SLOTS = m2k_lib.MAX_INVENTORY_SIZE
...
class Item:
	def __init__(self, id, slot, size, name):
		...
		self.name = name
	def move(self, slot):
		m2k_lib.DebugPrint("===> The item " + str(self.name) + " has been moved to slot " + str(slot))
...
class SortAlgorithm:
	...
	def retrieveItems(self):
		m2k_lib.DebugPrint("========= Retrieving item list ==========")
		...
		m2k_lib.DebugPrint("========= Retrieving successful ==========")
	...
	def sortByID(self):
		...
		m2k_lib.DebugPrint("========= Sorting successful ==========")
		...
		for page in range(int(math.ceil(SLOTS / SLOTS_PER_PAGE))):
			...
			for slot in range(page * SLOTS_PER_PAGE, LAST_SLOT_THIS_PAGE):
				if slot + 5 * (size - 1) < LAST_SLOT_THIS_PAGE:
					...
					if good:
						m2k_lib.DebugPrint("===> Best slot found is " + str(slot))
						return slot
		
		m2k_lib.DebugPrint("===> No free slots found of size " + str(size))
		return -1
	...
	def itemsInSlot(self, slot, size):
		...
		for i in range(size):
			...
			if len(it) > 0 and it[0] not in items:
				m2k_lib.DebugPrint("=====> The item " + str(it[0].name) + " is in the way" + str(size))
				items.append(it[0])
	...
	def getNextFreeSlot(self, size, skip = []):
		...
		m2k_lib.DebugPrint("Could not find any free slot")
		return -1        
	def setItemAsPlaced(self, item):
		m2k_lib.DebugPrint("===> The item has been placed in the right slot")
	...
	def sort(self,sortBy=3):
		...
		for it in self.list:
			for z in range(1000):
				continue
			m2k_lib.DebugPrint("Sorting Item " + str(it.name))
...
#####################INTERFACE#########################
#ATTENTION the callback will only be called after EVERY action has been done


def AddSellItems(slots,callback=None):
	for slot in slots:	
		inv.toSellSlots.append(slot)
	
	inv.addCallback(callback)

def AddBuyItems(slots,callback=None):
	for slot in slots:	
		inv.toBuySlots.append(slot)
	inv.addCallback(callback)

def AddDropItems(slots,callback=None):
	for slot in slots:	
		inv.toDropSlots.append(slot)
	inv.addCallback(callback)
