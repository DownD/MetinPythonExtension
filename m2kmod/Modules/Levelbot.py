import Movement
import m2k_lib,chr,player,net_packet,ui,chat,background
...
class LevelBotDialog(BotBase):
	...
	def BuildWindow(self):
		...
		self.EnableButton = comp.OnOffButton(self.Board, '', '', 0,323,OffUpVisual='m2kmod/Images/start_0.tga', OffOverVisual='m2kmod/Images/start_1.tga', OffDownVisual='m2kmod/Images/start_2.tga',OnUpVisual='m2kmod/Images/stop_0.tga', OnOverVisual='m2kmod/Images/stop_1.tga', OnDownVisual='m2kmod/Images/stop_2.tga',funcState=self.OnStartStop)
	...
	def UpdateLocationList(self):
		...
		for loc in sorted(self.locations,key=lambda x:x.level):
			self.fileListBox.AppendItem(m2k_lib.Item(str(loc)))
	...
	def GetNextMonster(self):
		...
		for vid in net_packet.InstancesList:
			...
			if self.ignoreBlockedPosition and net_packet.IsPositionBlocked(monst_pos[0],monst_pos[1]):
				continue

			if _type != m2k_lib.MONSTER_TYPE:
				continue

			if m2k_lib.dist(self.startPosition[0],self.startPosition[1],monst_pos[0],monst_pos[1]) > self.Range:
				continue
			
			this_dist = m2k_lib.dist(my_pos[0],my_pos[1],monst_pos[0],monst_pos[1])
