import ui,app,chr,net,player,item,skill,time,game,shop,thread,chat,net_packet,m2k_lib,Movement


#CHECK MORE THEN 45 SLOTS
#WON BUUG
BOARD_SIZE_X = 300
BOARD_SIZE_Y = 450
...
class SearchDialog(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.SearchButton = self.comp.OnOffButton(self.Board, '', '', relativeCenterX(-20),relativeEndY(55),func=self.EnableButtonClicked, OffUpVisual='m2kmod/Images/start_0.tga', OffOverVisual='m2kmod/Images/start_1.tga', OffDownVisual='m2kmod/Images/start_2.tga',OnUpVisual='m2kmod/Images/stop_0.tga', OnOverVisual='m2kmod/Images/stop_1.tga', OnDownVisual='m2kmod/Images/stop_2.tga' )
	...
	def EnableButtonClicked(self):
		if(self.SearchButton.isOn):
			self.SearchButton.SetOff()
			self.StopSearch()
		else:
			self.SearchButton.SetOn()
			self.StartSearch()
	...
	def ScanPlayers(self):
		for x in net_packet.InstancesList:
			...
			if test != 0 and chr.GetRace(x) >= m2k_lib.MIN_RACE_SHOP and chr.GetRace(x) <= m2k_lib.MAX_RACE_SHOP:
