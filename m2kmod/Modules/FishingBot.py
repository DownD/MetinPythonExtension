...
import m2k_lib,ShopNPC,FileManager,Movement
...
class FishingBotDialog(ui.Window):
	...
	def BuildWindow(self):
		...
		self.enableButton = self.comp.OnOffButton(self.Board, '', '', 165, 435, OffUpVisual='m2kmod/Images/start_0.tga', OffOverVisual='m2kmod/Images/start_1.tga', OffDownVisual='m2kmod/Images/start_2.tga',OnUpVisual='m2kmod/Images/stop_0.tga', OnOverVisual='m2kmod/Images/stop_1.tga', OnDownVisual='m2kmod/Images/stop_2.tga',func = self.StartStopEvent)
	...
	def loadSettings(self):
		for fish_id in self.catches.keys():
			for fishButtonName in self.catches[fish_id].keys():
				self.catches[fish_id][fishButtonName].SetValue(int(FileManager.ReadConfig("FishBot_" + str(fish_id) + "_" + fishButtonName)))
		...
		self.hairBtn.SetValue(int(FileManager.ReadConfig("FishBot_HairDyes")))
		self.buyWormsBtn.SetValue(int(FileManager.ReadConfig("FishBot_BuyWorms")))
		self.rodBtn.SetValue(int(FileManager.ReadConfig("FishBot_rodUpgrade")))


	def StartStopEvent(self):
	...
	def LevelUpRod(self):
		...
		val = m2k_lib.isItemTypeOnSlot(item.ITEM_TYPE_ROD,player.EQUIPMENT,item.EQUIPMENT_WEAPON)
		if val:
		...
		slot = m2k_lib.GetItemByType(item.ITEM_TYPE_ROD)
	...
	def stopFishing(self):
		if self.isRodDown:
			net_packet.SendStopFishing(net_packet.UNSUCCESS_FISHING,0)
			self.isRodDown = False
		net_packet.UnblockFishingPackets()
	...
	def PlaceFireAndGrillFish(self):
		slot = m2k_lib.GetItemByID(self.campFire)
		if slot != -1:
	...
	def OnUpdate(self):
		val, self.lastTime = m2k_lib.timeSleep(self.lastTime,0.1)
		if(val and self.enableButton.isOn and m2k_lib.IsInGamePhase()):
			if self.state == self.STATE_WAITING:
				valWaitState, self.lastTimeWaitState = m2k_lib.timeSleep(self.lastTimeWaitState,self.WaitDelaySlider.GetSliderPos()*10)
				if valWaitState:
					...
					self.lastTimeFishState = m2k_lib.GetTime()
					self.isRodDown = True
					self.isMoving = False
					
			if self.state == self.STATE_FISHING:
				valFishState, self.lastTimeFishState = m2k_lib.timeSleep(self.lastTimeFishState,self.StartStopDelaySlider.GetSliderPos()*10)
				if valFishState:
					chat.AppendChat(3,"[Fishing-Bot] Pulling Rod")
					net_packet.SendStopFishing(net_packet.SUCCESS_FISHING,self.ServerDelaySlider.GetSliderPos()*10)
					self.lastTimeWaitState = m2k_lib.GetTime()
					self.SetState(self.STATE_WAITING)
					self.isRodDown = False
			...
			if self.state == self.STATE_IMPROVING_ROD:
				val, self.lastTimeImprove = m2k_lib.timeSleep(self.lastTimeImprove,0.5)
				if not val:
					return
		...
		if self.state == self.STATE_PLACE_FIRE:
			val, self.lastTimeFire = m2k_lib.timeSleep(self.lastTimeFire,0.2)
			if not val:
				return
