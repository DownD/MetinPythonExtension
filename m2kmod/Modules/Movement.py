import net_packet,ui,chr,player,chat,background
import m2k_lib,MapManager
...
class MapMovementDialog(ui.ScriptWindow):
    ...
    #Either mapName or list must not be none Null
    def MoveToMapPosition(self,finalPosition,mapName=None,listLinks=[],callback=None,maxDist=250):
        if len(listLinks) == 0:
            if mapName == None or mapName == background.GetCurrentMapName():
                m2k_lib.DebugPrint("Moving to ("+str(finalPosition[0])+","+str(finalPosition[1])+") on " + str(background.GetCurrentMapName()))
            ...
            if len(listLinks) == 0:
                m2k_lib.DebugPrint("Error no map found by the name:"+str(mapName))
                return None
    ...
    def SetStateMoving(self):
        ...
        m2k_lib.DebugPrint("Moving to ("+str(position[0])+","+str(position[1])+")")
    ...
    def OnUpdate(self):
        if self.State == self.STATE_NONE or not m2k_lib.IsInGamePhase():
            return
        val, self.generalTimer = m2k_lib.timeSleep(self.generalTimer,TIME_WAIT_MAP_CHANGE)
        if not val:
            return
        if self.State == self.STATE_MOVING:
            m2k_lib.DebugPrint("[MAP] - State Moving")
        ...
        if self.State == self.STATE_MAPCHANGING:
            m2k_lib.DebugPrint("[MAP] - State Map Changing")
...
class MovementDialog(ui.ScriptWindow):
    ...
    def OnUpdate(self):
        val, self.generalTimer = m2k_lib.timeSleep(self.generalTimer,TIME_WAIT)
        if not val or not m2k_lib.IsInGamePhase():
            return

        m2k_lib.DebugPrint("[MOVEMENT] - State:" + str(self.state))
        ...
        if self.lastPlayerPos == (my_x,my_y):
            ...
            if val:
                #If is stuck
                self.path = net_packet.FindPath(my_x,my_y,self.currDestinationX,self.currDestinationY)
...
def TeleportToPosition(dst_x,dst_y,max_packets=MAX_TELEPORT_PACKETS):
    ...
    if len(points) == 0:
        return
    #chat.AppendChat(3,"Teleporting to X->" + str(dst_x) + "  Y->" + str(dst_y) + " Distance->" + str(m2k_lib.dist(x,y,dst_x,dst_y)) + " Number of Points ->"+ str(len(points)))
    curr_x,curr_y = (x,y)
...
def TeleportStraightLine(start_x,start_y,end_x,end_y,max_packets=MAX_TELEPORT_PACKETS):
    dst = m2k_lib.dist(start_x,start_y,end_x,end_y)
    step = int(dst/m2k_lib.MAX_TELEPORT_DIST)
    ...
    for i in range(0,step+1):
        _x,_y = m2k_lib.getPointsDistance(x,y,end_x,end_y,m2k_lib.MAX_TELEPORT_DIST)
        net_packet.SendStatePacket(_x,_y,0,net_packet.CHAR_STATE_STOP,0)
        #chat.AppendChat(3,"Straight Line Teleporting to X->" + str(_x) + "  Y->" + str(_y) + " Distance->" + str(m2k_lib.dist(x,y,_x,_y)))
        x,y = (_x,_y)
