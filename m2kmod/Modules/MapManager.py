from m2kmod.Modules.ShopNPC import NPCAction
import net_packet,m2k_lib,os,net,FileManager,player,chat,background,ShopNPC
...
class Map:
	...
	#recursive function that loops in depth until the level specified and returns
	#the path found in path_buffer. Is an interactive deep search
	#Improve possible by avoiding loops
	#def _GetClosestMapPath(self,dest_map_name,path_buffer,level):
	#	if dest_map_name == self.map_name:
	#		return True
	#	if level == 0:
	#		return None
	#	for map_name,link in self.links.iteritems():
	#		if(link.map_dest._GetClosestMapPath(dest_map_name,path_buffer,level-1) != None):
	...
	#the path found in path_buffer. Is an interactive deep search
	#Improve possible by avoiding loops
	#def _GetClosestMapPathWithNPC(self,npc_race,path_buffer,level):
	#	if npc_race in self.npcs:
	#		return True
	#	if level == 0:
	#		return None
	#	for map_name,link in self.links.iteritems():
	#		if(link.map_dest._GetClosestMapPath(npc_race,path_buffer,level-1) != None):
	#			path_buffer.append(link)
	#			return True
	...
	def GetNpcPositionClosest(self,npc_race,curr_x,curr_y):
		...
		for x,y in self.npcs[npc_race]:
			dst = m2k_lib.dist(curr_x,curr_y,x,y)
			if dst<min_dist:
				min_dist = dst
				x_result = x
				y_result = y
...
#Returns empty list if map is not configured
...
def GetClosestMapPathWithNPC(npc_race,map_name_start=background.GetCurrentMapName()):
...
def GetNpcFromMap(map_name,npc_race,position=player.GetMainCharacterPosition()):
	#m2k_lib.DebugPrint("Searching NPC race:"+str(npc_race) + " on map " + str(map_name))
	map = GetMap(map_name)
